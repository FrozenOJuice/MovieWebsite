...
def update_my_profile(update: schemas.UserSelfUpdate, current_user: schemas.UserToken = Depends(get_current_user)):
    return utils.update_user(current_user.user_id, update.dict(exclude_unset=True))


@router.patch("/me/password")
def change_my_password(update: schemas.PasswordChange, current_user: schemas.UserToken = Depends(get_current_user)):
    utils.change_password(current_user.user_id, update.old_password, update.new_password)
    return {"message": "Password updated successfully."}


@router.patch("/me/status")
def change_my_status(update: schemas.StatusUpdate, current_user: schemas.UserToken = Depends(get_current_user)):
...
def update_user_admin(user_id: str, update: schemas.UserAdminUpdate, current_user: schemas.UserToken = Depends(get_current_user)):
    require_role(current_user, ["administrator"])
    return utils.update_user(user_id, update.dict(exclude_unset=True))


@router.delete("/{user_id}")
def delete_user(user_id: str, current_user: schemas.UserToken = Depends(get_current_user)):
    require_role(current_user, ["administrator"])
